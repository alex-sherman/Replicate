...
namespace Replicate
{
    ...
    {
        public static void Respond(this IRPCServer channel, HandlerDelegate handler)
        {
            channel.Respond(handler.Method, handler);
        }
        public static void Respond<TRequest, TResponse>(this IRPCServer channel, Func<TRequest, Task<TResponse>> handler)
        {
            channel.Respond(handler.Method, async (req) => { return await handler(TypeUtil.Cast<TRequest>(req.Request)); });
        }
        public static void Respond<TRequest>(this IRPCServer channel, Action<TRequest> handler)
        {
